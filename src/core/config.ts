...
export interface EnvConfigData<T = any> {
    required?: boolean;
    default?: string | number | boolean;
    filter?: (value: string | undefined) => T | undefined;
    verify?: (value: string | T | undefined) => void | never;
}
...
export const DefaultConfigSet: { [K in ConfigKeys]: EnvConfigData } = {
    APP_SECRET: <EnvConfigData<string>>{
        ...
        verify: (secret): void | never => {
            if (!secret) { // noinspection ExceptionCaughtLocallyJS
    ...
    APP_DEBUG: <EnvConfigData<boolean>>{
        default: false,
        filter: (v) => {
            return !!(v && (['true', '1'].includes(v.toLowerCase())));
        }
    }
}
